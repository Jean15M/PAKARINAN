...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	<div class="blogger">
		<?php $foster_charity_blog_layout = get_theme_mod( 'foster_charity_blog_post_layout','Default');
		if($foster_charity_blog_layout == 'Default'){ ?>
			...
			<div class="row">
				<div class="row">
        <?php if(has_post_thumbnail() && get_theme_mod( 'foster_charity_blog_post_featured_image',true) != '') { ?>
		 		...
		 		<?php } ?>
		 		<div class="<?php if(has_post_thumbnail() && get_theme_mod( 'foster_charity_blog_post_featured_image',true) != '') { ?>col-lg-8 col-md-12"<?php } else { ?>col-lg-12 col-md-12" <?php } ?>>
				 	<?php if(get_theme_mod('foster_charity_blog_description') == 'Post Content'){ ?>
			          ...
			          if(get_theme_mod('foster_charity_blog_description', 'Post Excerpt') == 'Post Excerpt'){ ?>
			            <?php if(get_the_excerpt()) { ?>
		              		<div class="text inicap"><?php $foster_charity_excerpt = get_the_excerpt(); echo esc_html( foster_charity_string_limit_words( $foster_charity_excerpt, esc_attr(get_theme_mod('foster_charity_excerpt_number','20')))); ?><?php echo esc_html( get_theme_mod('foster_charity_post_excerpt_suffix','{...}') ); ?></div>
			            <?php } ?>
			        <?php }?>
			...
			<?php } ?>
		<?php }else if($foster_charity_blog_layout == 'Center' || $foster_charity_blog_layout == 'Left'){ ?>
			<h2><a href="<?php echo esc_url( get_permalink() ); ?>" title="<?php echo the_title_attribute(); ?>" class="text-capitalize"><?php the_title();?><span class="screen-reader-text"><?php the_title(); ?></span></a></h2>
			<div class="row">
        <?php if(has_post_thumbnail() && get_theme_mod( 'foster_charity_blog_post_featured_image',true) != '') { ?>
				<div class="post-image">
